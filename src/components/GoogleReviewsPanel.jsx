  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 24 }}>
        <div style={{ background: 'var(--pn-surface)', border: '1px solid var(--pn-border)', borderRadius: 12, padding: '16px 18px' }}>
          <div style={{ fontSize: 11, color: 'var(--pn-text-faint)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>Reviews received</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#2D7A5F' }}>{stats.totalReceived}</div>
        </div>
        <div style={{ background: 'var(--pn-surface)', border: '1px solid var(--pn-border)', borderRadius: 12, padding: '16px 18px' }}>
          <div style={{ fontSize: 11, color: 'var(--pn-text-faint)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>Requests sent</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#3D95CE' }}>{stats.totalRequested}</div>
        </div>
        <div style={{ background: 'var(--pn-surface)', border: '1px solid var(--pn-border)', borderRadius: 12, padding: '16px 18px' }}>
          <div style={{ fontSize: 11, color: 'var(--pn-text-faint)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>Conversion rate</div>
          ...
          {stats.totalRequested > 0 && (
            <div style={{ fontSize: 11, color: 'var(--pn-text-faint)', marginTop: 3 }}>{stats.totalReceived} of {stats.totalRequested} requests led to a review</div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        ...
        <div style={{ background: 'var(--pn-surface)', border: '1px solid var(--pn-border)', borderRadius: 12, padding: '16px 18px' }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--pn-text-faint)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 14 }}>By tech</div>
          {stats.leaderboard.length === 0 ? (
            ...
            <div key={t.name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: i < stats.leaderboard.length - 1 ? '1px solid var(--pn-border)' : 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--pn-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.name}</div>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--pn-text-faint)' }}>{t.avg.toFixed(1)} avg</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: 'var(--pn-surface)', border: '1px solid var(--pn-border)', borderRadius: 12, padding: '16px 18px' }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--pn-text-faint)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 14 }}>Recent reviews</div>
          {received.length === 0 ? (
            <div style={{ fontSize: 13, color: 'var(--pn-text-faint)', textAlign: 'center', padding: '20px 0' }}>No reviews recorded yet</div>
          ) : received.slice(0, 8).map((r, i) => (
            <div key={r.id} style={{ padding: '8px 0', borderBottom: i < Math.min(received.length, 8) - 1 ? '1px solid var(--pn-border)' : 'none' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--pn-text)' }}>{r.clientName || 'Client'}</div>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--pn-text-faint)', marginTop: 1 }}>{fmtDate(r.confirmedAt || r.createdAt)}</div>
                </div>
              </div>
              {r.text && <div style={{ fontSize: 11, color: 'var(--pn-text-muted)', marginTop: 3, fontStyle: 'italic', lineHeight: 1.4 }}>"{r.text.slice(0, 80)}{r.text.length > 80 ? '…' : ''}"</div>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: 'var(--pn-surface)', border: '1px solid var(--pn-border)', borderRadius: 12, padding: '16px 18px' }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--pn-text-faint)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 14 }}>Review requests sent</div>
        {requests.filter(r => r.sent).length === 0 ? (
          <div style={{ fontSize: 13, color: 'var(--pn-text-faint)', textAlign: 'center', padding: '20px 0' }}>No requests sent yet. Use the "Request Review" button after checkout.</div>
        ) : (
          <div>
            {requests.filter(r => r.sent).slice(0, 20).map((r, i, arr) => {
              const gotReview = received.some(rv => rv.clientId === r.clientId);
              return (
                <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: i < arr.length - 1 ? '1px solid var(--pn-border)' : 'none' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--pn-text)' }}>{r.clientName || 'Client'}</div>
