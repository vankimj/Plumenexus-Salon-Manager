...
import { computeTechSlice } from '../lib/earnings';
...
export default function EarningsPanel({ techName }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const receipts = await fetchReceiptsByRange(startDate, endDate);
        if (alive) setData(computeTechSlice(receipts, techName));
